...
const createWorkspaceRequestSchema = z.object({
	name: z.string().min(1, { message: MIN_NAME_MESSAGE }).max(50, { message: MAX_NAME_MESSAGE }),
});
...
const exchangeCredentialsRequestSchema = z.object({
	username: z.string().email({ message: "Invalid username. Must be an email address" }),
	password: z.string(),
...
const updateUserRequestSchema = z.object({
	...
	name: z.string().max(50, { message: MAX_NAME_MESSAGE }).or(z.string().optional()),
	phone: z.string().max(20).optional(),
	password: z.string().min(8, { message: PASSWORD_MESSAGE }).or(z.string().optional()),
