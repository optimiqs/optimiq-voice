...
describe("@api[common/assistants/assistantSchema]", function () {
  ...
  it("checks the tools have an empty array by default", async function () {
    ...
    expect(result.conversationSettings.vad.activationThreshold).to.be.equal(
      0.4
    );
    expect(result.conversationSettings.vad.deactivationThreshold).to.be.equal(
      0.25
    );
    expect(result.conversationSettings.vad.debounceFrames).to.be.equal(4);
    expect(result.conversationSettings.transferOptions.timeout).to.be.equal(
      30000
    );
    ...
    expect(result.conversationSettings.idleOptions.maxTimeoutCount).to.be.equal(
      2
    );
  });
});
