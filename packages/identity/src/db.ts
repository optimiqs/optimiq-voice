...
type UserWhere = ScalarWhere<Pick<User, "accessKeyId" | "email" | "phoneNumber" | "ref">>;
type WorkspaceWhere = ScalarWhere<Pick<Workspace, "ownerRef" | "ref">> & {
	OR?: WorkspaceWhere[];
	accessKeyId?: string | { in?: string[] };
	members?: {
		some: ScalarWhere<Pick<WorkspaceMember, "role" | "status" | "userRef" | "workspaceRef">>;
	};
};
type WorkspaceMemberWhere = ScalarWhere<
	Pick<WorkspaceMember, "ref" | "role" | "status" | "userRef" | "workspaceRef">
>;
type ApiKeyWhere = ScalarWhere<Pick<ApiKey, "accessKeyId" | "ref" | "role" | "workspaceRef">>;
...
type WorkspaceMemberUpdateInput = Partial<Omit<WorkspaceMemberCreateInput, "ref">>;
...
type VerificationCodeUpdateInput = Partial<Omit<VerificationCodeCreateInput, "ref">>;
...
type Delegate<R, C, U, W> = {
	...
	upsert(args: QueryOptions<W> & { create: C; update: U; where: W }): Promise<R>;
};

type Database = {
	apiKey: Delegate<ApiKeyResult, ApiKeyCreateInput, ApiKeyUpdateInput, ApiKeyWhere>;
	close(): Promise<void>;
	...
	>;
	workspace: Delegate<WorkspaceResult, WorkspaceCreateInput, WorkspaceUpdateInput, WorkspaceWhere>;
...
function createEncryption(cloakEncryptionKey: string): Encryption {
	const fallbackKeys = (process.env.PRISMA_FIELD_DECRYPTION_KEYS ?? "").split(",").filter(Boolean);
...
function decrypt(value: string | null | undefined, encryption: Encryption) {
	...
	try {
		...
		console.error(`[database-encryption] Error: decryption error(s) encountered: ${error}`);
		return value;
	}
}
...
function workspaceCondition(
	...
	if (where.OR) {
		conditions.push(or(...where.OR.map((item) => workspaceCondition(db, item))));
	}
...
function withCursor(condition: SQL | undefined, column: AnyPgColumn, cursor?: { ref: string }) {
...
function project<T extends Record<string, unknown>>(
	...
	return Object.fromEntries(Object.entries(record).filter(([key]) => select[key])) as T;
}
...
function createDatabase(
	...
	async function includeWorkspaceMember(
		...
		if (include?.user) {
			const [user] = await db.select().from(users).where(eq(users.ref, member.userRef)).limit(1);
	...
	const user: Database["user"] = {
		...
		async delete(args) {
			return execute(async () => {
				const [record] = await db.delete(users).where(userCondition(args.where)).returning();
				if (!record) {
		...
		async update(args) {
			return execute(async () => {
				const data = compact({
					...args.data,
					password:
						args.data.password === undefined ? undefined : encrypt(args.data.password, encryption),
				});
	...
	const workspace: Database["workspace"] = {
		async create(args) {
			return execute(async () => {
				const [record] = await db
					...
					.returning();
				return project(await includeWorkspace(record, args.include), args.select);
			});
		},
		async delete(args) {
			return execute(async () => {
		...
		async findMany(args = {}) {
			return execute(async () => {
				let query = db
					.select()
					.from(workspaces)
					.where(withCursor(workspaceCondition(db, args.where), workspaces.ref, args.cursor))
					.$dynamic();
		...
		async upsert(args) {
			return execute(async () => {
				...
				return project(await includeWorkspace(records[0], args.include), args.select);
			});
		},
	};
	...
	const workspaceMember: Database["workspaceMember"] = {
		async create(args) {
			return execute(async () => {
				const [record] = await db
					...
					.values(compact({ ...args.data, ref: args.data.ref ?? randomUUID() }))
					.returning();
				return project(await includeWorkspaceMember(record, args.include), args.select);
			});
		},
		async delete(args) {
			return execute(async () => {
				const [record] = await db
					...
					.returning();
				if (!record) {
					throw createDatabaseError(DATABASE_NOT_FOUND, "Workspace member not found");
				}
				return project(await includeWorkspaceMember(record, args.include), args.select);
			});
		},
		...
		async findMany(args = {}) {
			return execute(async () => {
				let query = db
					...
					.from(workspaceMembers)
					.where(
						withCursor(workspaceMemberCondition(args.where), workspaceMembers.ref, args.cursor),
					)
					.$dynamic();
				...
				return Promise.all(
					records.map(async (record) =>
						project(await includeWorkspaceMember(record, args.include), args.select),
					),
				);
			});
		},
		...
		async upsert(args) {
			return execute(async () => {
				const [record] = await db
					.insert(workspaceMembers)
					.values(compact({ ...args.create, ref: args.create.ref ?? randomUUID() }))
					.onConflictDoUpdate({
	...
	const apiKey: Database["apiKey"] = {
		...
		async delete(args) {
			return execute(async () => {
				const [record] = await db.delete(apiKeys).where(apiKeyCondition(args.where)).returning();
				if (!record) {
		...
		async findMany(args = {}) {
			return execute(async () => {
				let query = db
					.select()
					.from(apiKeys)
					.where(withCursor(apiKeyCondition(args.where), apiKeys.ref, args.cursor))
					.$dynamic();
		...
		async delete(args) {
			return execute(async () => {
				const [record] = await db
					...
					.returning();
				if (!record) {
					throw createDatabaseError(DATABASE_NOT_FOUND, "Verification code not found");
				}
				return project(record, args.select);
			});
		},
		...
		async findMany(args = {}) {
			return execute(async () => {
				let query = db
					...
					.where(
						withCursor(verificationCodeCondition(args.where), verificationCodes.ref, args.cursor),
					)
					.$dynamic();
...
function createDatabaseClient(dbUrl: string, cloakEncryptionKey: string): Database {
...
export { createDatabaseClient, Database, DATABASE_ALREADY_EXISTS, DATABASE_NOT_FOUND };
