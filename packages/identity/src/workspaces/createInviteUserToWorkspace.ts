...
import { AccessKeyIdType, createSendEmail, generateAccessKeyId } from "../utils";
...
function createInviteUserToWorkspace(
	...
	const inviteUserToWorkspace = async (
		...
		callback: (error: GrpcErrorMessage, response?: InviteUserToWorkspaceResponse) => void,
	) => {
		...
		const inviteeToken = await createGenerateWorkspaceInviteToken(identityConfig)({
			userRef: user.ref,
