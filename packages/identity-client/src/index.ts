...
const IdentityService = proto.optimiq_voice.identity.v1beta2.Identity;
...
/**
 * Lightweight, stateless client for the Optimiq Voice Identity gRPC service. Targets
...
export class IdentityClient {
	 ...
	 */
	exchangeApiKey(accessKeyId: string, accessKeySecret: string): Promise<ExchangeResponse> {
		return this.unary("exchangeApiKey", { accessKeyId, accessKeySecret });
	}

	/** Exchange an OAuth2 authorization code for tokens (sign in). */
	exchangeOauth2Code(provider: Oauth2Provider, code: string): Promise<ExchangeResponse> {
	...
	regenerateApiKey(ref: string, accessKeyId: string, token: string): Promise<ApiKeyCredentials> {
		return this.unary("regenerateApiKey", { ref }, { token, accessKeyId });
	}

	/** Permanently delete a key (this is the revocation mechanism). */
	deleteApiKey(ref: string, accessKeyId: string, token: string): Promise<{ ref: string }> {
