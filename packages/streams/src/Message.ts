...
import { ErrorCode, MAXIMUM_MESSAGE_LENGTH, MessageType, MINIMUM_MESSAGE_LENGTH } from "./types";
...
class Message {
	...
	getContentLength(): number {
		return this.data.length >= MINIMUM_MESSAGE_LENGTH ? this.data.readUInt16BE(1) : 0;
	}
	...
	getErrorCode(): ErrorCode {
		...
		return this.data.length >= 4 ? this.data[MINIMUM_MESSAGE_LENGTH] : ErrorCode.UNKNOWN;
	}
