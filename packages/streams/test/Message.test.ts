...
describe("@streams/Message", function () {
	...
	beforeEach(function () {
		...
		message = new Message(Buffer.from([MessageType.ID, 0x00, 0x02, 0x03, 0x04]));
	});
	...
	it("should return the uuid of the stream", function () {
		...
		// Assert
		expect(idMessage.getId()).to.be.equal("36c3a775-0414-4af2-98ae-2e7cdb65cf65");
	});

	it("should throw an error when the message type is wrong", function () {
		// Arrange
		const idMessage = new Message(Buffer.from([0x03, 0x00, 0x10, 0x01, 0x02, 0x03, 0x04]));
	...
	it("should create a hangup message", function () {
		// Assert
		expect(Message.createHangupMessage()).to.be.eql(Buffer.from([MessageType.HANGUP, 0x00, 0x00]));
	});
