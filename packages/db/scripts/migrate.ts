 ...
 * it needs are the owner connection string and the stage guards.
 */
...
function requireMigrationUrl(): string {
	...
	if (!url) {
		throw new MigrationTargetError(
			"DATABASE_MIGRATION_URL (preferred) or DATABASE_URL must be set for migrations.",
		);
	}
	return url;
}
...
async function main(): Promise<void> {
	...
	process.stdout.write(
		`${JSON.stringify({ event: "database_migration_start", stage: actualStage, target })}\n`,
	);
	...
	process.stdout.write(
		`${JSON.stringify({ event: "database_migration_complete", stage: actualStage, target })}\n`,
	);
}
