...
/**
 * Telephony schema migration entry point. Same stage guard as the base package, aimed at the PBX
 * connection string: `PBX_DATABASE_MIGRATION_URL` is the owner principal and takes precedence over
 * the runtime `PBX_DATABASE_URL`.
 *
 * Reads `process.env` directly rather than @optimiq-voice/config: this is a bootstrap script that
 * must run in release images where the full application env is not present.
 */
...
function requireMigrationUrl(): string {
	...
	if (!url) {
		throw new MigrationTargetError(
			"PBX_DATABASE_MIGRATION_URL (preferred) or PBX_DATABASE_URL must be set for telephony migrations.",
		);
	}
	return url;
}
...
async function main(): Promise<void> {
	...
	process.stdout.write(
		`${JSON.stringify({ event: "pbx_migration_start", stage: actualStage, target })}\n`,
	);

	const client = createPostgresClient({
		url,
		applicationName: "optimiq-voice-pbx-migrator",
	...
	process.stdout.write(
		`${JSON.stringify({ event: "pbx_migration_complete", stage: actualStage, target })}\n`,
	);
}
