...
/**
 * @classdesc Optimiq Voice Agents, part of the Optimiq Voice SIP Proxy subsystem,
...
class Agents {
	...
	async getAgent(ref: string): Promise<Agent> {
		const client = this.client.getAgentsClient();
		const response = await makeRpcRequest<GetAgentRequestPB, AgentPB, BaseApiObject, Agent>({
