...
import { DataResponse, OptimiqVoiceClient } from "./client/types";
import {
	CallDirection,
	Call as CallPB,
	CallStatus,
...
/**
 * @classdesc Optimiq Voice Calls, part of the Optimiq Voice Media subsystem,
...
class Calls {
	...
	async createCall(request: CreateCallRequest): Promise<{
		...
		>({
			...
			request: {
				...request,
				metadata: Struct.fromJavaScript(request.metadata) as unknown as Record<string, unknown>,
			},
		});
