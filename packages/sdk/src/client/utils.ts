...
function getEnumValue(key: string, value: string, enumMapping: MappingTuple<unknown>): number {
...
function getEnumKey(key: string, value: number, enumMapping: MappingTuple<unknown>): string {
	const tuple = enumMapping.find((tuple) => tuple[0] === key);
