...
/**
 * @classdesc Optimiq Voice Applications, part of the Optimiq Voice Voice Subsystem,
...
class Applications {
	 ...
	 */
	async createApplication(request: CreateApplicationRequest): Promise<BaseApiObject> {
	 ...
	 *   .catch(console.error); // an error occurred
	 */
	async updateApplication(request: UpdateApplicationRequest): Promise<BaseApiObject> {
		const reqWithStructOverride = buildStructOverride(request);
		const applicationsClient = this.client.getApplicationsClient();
		...
		getStepResult?: () => {
			...
			getReport: () => {
				...
				getToolEvaluationsList: () => Array<{
					...
					getPassed: () => boolean;
					getExpectedParameters: () => { toJavaScript: () => Record<string, unknown> } | undefined;
					getActualParameters: () => { toJavaScript: () => Record<string, unknown> } | undefined;
					getErrorMessage: () => string;
				}>;
			};
		};
		...
		if (msg.getStepResult?.()) {
			...
			return {
				...
				stepResult: {
					...
					evaluationType:
						report.getEvaluationType() === 0 ? ExpectedTextType.EXACT : ExpectedTextType.SIMILAR,
