...
/**
 * @classdesc Optimiq Voice Trunks, part of the Optimiq Voice SIP Proxy subsystem,
	...
	async getTrunk(ref: string): Promise<Trunk> {
		...
		return new Promise((resolve, reject) => {
			client.getTrunk(getTrunkRequest, this.client.getMetadata(), (err, response) => {
				...
				const { urisList, accessControlList, inboundCredentials, outboundCredentials, ...rest } =
					outObj;
