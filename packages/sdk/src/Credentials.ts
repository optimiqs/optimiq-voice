...
/**
 * @classdesc Optimiq Voice Credentials, part of the Optimiq Voice SIP Proxy subsystem,
...
class Credentials {
	 ...
	 */
	async createCredentials(request: CreateCredentialsRequest): Promise<BaseApiObject> {
	 ...
	 *   .catch(console.error); // an error occurred
	 */
	async updateCredentials(request: UpdateCredentialsRequest): Promise<BaseApiObject> {
		const client = this.client.getCredentialsClient();
		return await makeRpcRequest<
