...
/**
 * @classdesc Optimiq Voice Numbers, part of the Optimiq Voice SIP Proxy subsystem,
...
class Numbers {
	...
	async getNumber(ref: string): Promise<INumber> {
		const client = this.client.getNumbersClient();
		const response = await makeRpcRequest<GetNumberRequestPB, NumberPB, BaseApiObject, INumber>({
