		...
		cases: [
			...
			},
			{
				...
				responseValidator: (response: { items: unknown[]; nextPageToken: string }) => {
					...
					expect(response.items[0]).to.have.property("type").to.be.equal("API_ORIGINATED");
					...
					expect(response.items[0]).to.have.property("startedAt").to.be.a("date");
					...
					expect(response.items[0]).to.have.property("duration").to.be.a("number");
					expect(response.items[0]).to.have.property("direction").to.be.equal("TO_PSTN");
				},
				skip: true,
			},
		],
