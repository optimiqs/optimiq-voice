function createNumbersTestCases(expect) {
	...
	const agentAor = "sip:1001@sip.optimiq-voice.local";
		...
		cases: [
			...
			},
			{
				...
				responseValidator: (response: { items: unknown[]; nextPageToken: string }) => {
					...
					expect(response.items[0]).to.have.property("name").to.be.equal("My New Number");
					expect(response.items[0]).to.have.property("telUrl").to.not.be.null;
					expect(response.items[0]).to.have.property("city").to.be.equal("Asheville");
					expect(response.items[0]).to.have.property("country").to.be.equal(country);
					expect(response.items[0]).to.have.property("countryIsoCode").to.be.equal("US");
					expect(response.items[0]).to.have.property("agentAor").to.be.equal(agentAor);
