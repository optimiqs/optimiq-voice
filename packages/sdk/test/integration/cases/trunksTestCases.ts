		...
		cases: [
			{
				...
				request: {
					name: "My Trunk",
					inboundUri: `${Math.floor(Math.random() * (999 + 1)) + 1}.test.pstn.optimiq.health`,
					sendRegister: true,
					uris: [
						{
							host: exampleHost,
			...
			},
			{
				...
				responseValidator: (response: { ref: string; uris: unknown[] }) => {
					...
					expect(response).has.property("uris").to.be.an("array").to.have.lengthOf(1);
					expect(response.uris[0]).has.property("host").to.be.equal(exampleHost);
			...
			},
			{
				...
				responseValidator: (response: { items: { uris: unknown[] }[]; nextPageToken: string }) => {
					...
					expect(response.items[0]).to.have.property("inboundUri").to.not.be.null;
					expect(response.items[0]).to.have.property("sendRegister").to.be.true;
					expect(response.items[0]).to.have.property("uris").to.be.an("array").to.have.lengthOf(1);
					expect(response.items[0].uris[0]).has.property("host").to.be.equal(exampleHost);
					expect(response.items[0].uris[0]).has.property("port").to.be.equal(5060);
					expect(response.items[0].uris[0]).has.property("transport").to.be.equal("TCP");
					expect(response.items[0].uris[0]).has.property("user").to.be.equal("1001");
					expect(response.items[0].uris[0]).has.property("weight").to.be.equal(1);
					expect(response.items[0].uris[0]).has.property("priority").to.be.equal(1);
