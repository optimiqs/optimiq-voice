...
function listTrunks(trunks: TrunkApi) {
	return listResources<TrunkExtended, ListTrunksRequest, TrunkApi>(trunks, RESOURCE);
}

function deleteTrunk(trunks: TrunkApi) {
	return deleteResource<TrunkExtended, BaseApiObject, TrunkApi>(trunks, RESOURCE);
}
