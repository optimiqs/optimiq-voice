...
export class GoFileEmitter {
	...
	render(packageDoc: readonly string[], packageName: string): string {
		const lines: string[] = [
			// Line 1 is the canonical form Go tooling matches; keep it verbatim.
	...
	private pushEnum(name: string, doc: readonly string[], values: readonly string[]): void {
		...
		lines.push(`func (v ${name}) Valid() bool { return slices.Contains(${name}Values, v) }`, "");
		this.imports.add("slices");
