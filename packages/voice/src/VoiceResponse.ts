...
class VoiceResponse {
	 ...
	 * async function handler (request, response) {
	 *    await response.answer();
	 *    await response.play("https://s3.optimiq.health/uuid/hello-world.wav", { playbackRef: "playback-01" });
	 *
	 *    // Pause the media
	 *    await response.playbackControl("playback-01", PlaybackControlAction.PAUSE);
	 * }
	 */
	async playbackControl(playbackRef: string, action: PlaybackControlAction): Promise<VerbResponse> {
	 ...
	 */
	async dial(destination: string, options?: DialOptions): Promise<DialStatusStream> {
	 ...
	 * }
	 */
	async mute(options: MuteOptions = { direction: MuteDirection.BOTH }): Promise<void> {
	 ...
	 * }
	 */
	async unmute(options: MuteOptions = { direction: MuteDirection.BOTH }): Promise<void> {
		const { direction } = options;
