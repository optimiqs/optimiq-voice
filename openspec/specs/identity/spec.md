...
Optimiq Voice Identity is the cornerstone for user management, authentication, and authorization across
Optimiq Voice products. It manages users and workspaces, issues and verifies RS256 JWTs (id/access/refresh),
and enforces role-based access control over the `optimiq_voice.identity.v1beta2.Identity` gRPC surface.

This spec was seeded from `packages/identity/README.md` to capture the current, established behavior.
...
(e.g. `/optimiq_voice.identity.v1beta2.Identity/GetUser`). Predefined roles (e.g. owner, admin, member)
...
`optimiq_voice.identity.v1beta2.Identity.GetPublicKey` gRPC method. Verification SHALL confirm the token
