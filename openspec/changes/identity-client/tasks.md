...
- [ ] 1.1 Treat `packages/identity`'s Identity proto as the single source; expose it as a build input for the client package
- [ ] 1.2 Set up gRPC/TS codegen producing client stubs into `packages/identity-client`

## 2. Lightweight client package

- [ ] 2.1 Create `packages/identity-client` (`@optimiq-voice/identity-client`) depending only on `@grpc/grpc-js` (+ proto runtime) — no ORM, no server, no telephony deps
...
- [ ] 3.2 Installing `@optimiq-voice/identity-client` pulls neither an ORM nor any server/telephony runtime
