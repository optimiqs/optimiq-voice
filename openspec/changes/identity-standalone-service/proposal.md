...
`@optimiq-voice/identity` is a library (`buildIdentityService`) with no runnable form of its own.
The only thing that runs it is `apps/api`, which couples Identity to the telephony
stack (Asterisk/ARI, NATS, InfluxDB). External products that want to reuse Optimiq Voice Identity
(e.g. QCobro) are forced to hand-wire their own gRPC server around `buildIdentityService`,
vendor the proto, and vendor a copy of the database schema to provision the database — duplicating
...
- **Make `packages/identity` runnable.** Add a server entrypoint to the existing `@optimiq-voice/identity`
  package — exposed via a `./server` subpath export and a `serve` bin — that wraps
  `buildIdentityService` into a gRPC server with the same cross-cutting concerns the api
  ...
  pure library, so existing importers (e.g. `api`) are unaffected. No new package is created.
...
- **Owned database provisioning.** The service ships the Drizzle schema + migrations and a
...
- `identity-service`: Running Optimiq Voice Identity as a standalone gRPC service — its server
  bootstrap, auth/allow-list, accept-invite bridge, health, configuration, database provisioning,
  and container packaging.

## Impact

- **Code:** extend `packages/identity` with a runnable server entrypoint (a `./server` subpath export
  - a `serve` bin) so the package stays importable as a pure library; add its Dockerfile and a
    ...
    identity allow-list and accept-invite bridge from `apps/api` into a shared/reusable
    location (`packages/identity` or `packages/common`) so the monolith and the standalone runner use one
    source. No new package.
- **Release:** CI to build and publish the image (e.g. `ghcr.io/optimiq-voice/identity`).
- **Docs:** how to run and configure the service.
- **Compatibility:** `apps/api` continues to work unchanged (it may optionally consume the
  extracted helpers). The standalone service exposes the same `optimiq_voice.identity.v1beta2` surface.
