...
Optimiq Voice SHALL provide a runnable service that serves the Identity gRPC surface
(`optimiq_voice.identity.v1beta2`) on its own, built from `buildIdentityService`, without starting any
...
- **THEN** it binds the configured address and serves the `optimiq_voice.identity.v1beta2.Identity` methods
...
- **WHEN** a consumer imports the `@optimiq-voice/identity` main entry as a library (e.g. `buildIdentityService`)
...
The service SHALL ship the Identity Drizzle schema and migrations and provide an entrypoint that
