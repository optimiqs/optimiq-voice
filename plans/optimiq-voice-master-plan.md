...
5. **D5 — RESOLVED (owner, 2026-08-06): Telnyx is the carrier layer** for phone-number management and PSTN calling services. Number search/order/release and number configuration go through the Telnyx API (org-scoped, keys in platform config, never per-tenant browser-side); inbound origination and outbound termination ride Telnyx SIP trunking (credential-based connections mapped to our trunk entity; Telnyx-sourced inbound feeds the DID→org index). The trunk model stays carrier-agnostic — Telnyx is the first-class managed provider, not a hard dependency; BYO-SIP trunks keep working.
