...
import { ExpectedTextType, ScenarioEvaluationReport } from "@optimiq-voice/types";
...
function findApiKey(integrations: Integration[], productRef: string): string | undefined {
	return integrations.find((i) => i.productRef === productRef)?.credentials?.apiKey;
}
 ...
 */
function injectCredentials(config: AssistantRawConfig, integrations: Integration[]): void {
 ...
 */
function normalizeExpectedTextTypes(scenarios: TestCasesConfig["scenarios"]): void {
...
describe("Autopilot Evaluation Scenarios", function () {
	...
	it("should run all scenarios successfully", function () {
		expect(reports, "evaluation reports should exist").to.be.an("array").that.is.not.empty;
	});
	...
	it("should pass conversation-termination with a hangup tool call", function () {
		const termination = reports.find((r) => r.scenarioRef === "conversation-termination");
		expect(termination, "conversation-termination scenario should exist").to.exist;
		expect(termination!.overallPassed, formatFailureMessage(termination!)).to.be.true;

		// Verify the hangup tool was actually invoked
		const lastStep = termination!.steps?.[termination!.steps.length - 1];
		expect(lastStep?.toolEvaluations, "last step should have tool evaluations").to.be.an("array")
			.that.is.not.empty;
		expect(lastStep?.toolEvaluations?.[0]?.passed, "hangup tool evaluation should pass").to.be.true;
	});
});
