...
import { Anthropic, LANGUAGE_MODEL_NAME as ANTHROPIC_LLM_NAME, AnthropicParams } from "./anthropic";
import { Google, LANGUAGE_MODEL_NAME as GOOGLE_LLM_NAME, GoogleParams } from "./google";
import { Groq, LANGUAGE_MODEL_NAME as GROQ_LLM_NAME, GroqParams } from "./groq";
import { Ollama, LANGUAGE_MODEL_NAME as OLLAMA_LLM_NAME, OllamaParams } from "./ollama";
import { OpenAI, LANGUAGE_MODEL_NAME as OPENAI_LLM_NAME, OpenAIParams } from "./openai";
import { BaseModelParams, TelephonyContext } from "./types";
const logger = getLogger({ service: "autopilot", filePath: __filename });

type LanguageModelConstructor<T extends BaseModelParams = BaseModelParams> = new (
	options: T,
	voice: Voice,
...
class LanguageModelFactory {
	private static readonly languageModels: Map<string, LanguageModelConstructor> = new Map();
