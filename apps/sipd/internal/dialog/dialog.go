...
const (
	...
	// RFC's instruction is to send a BYE, with a timer-expiry cause rather than normal clearing.
	TimeoutAck
	// TimeoutSession is RFC 4028 §10: the session-expiry deadline passed without a refresh, so a BYE
	// goes out.
	TimeoutSession
...
// Input is one trigger plus everything the machine needs to decide the effects. One struct rather
// than a method per trigger, because the owner reads its inputs off a single mailbox channel
// (design §4.4) and that single ordering is what makes a race have one winner.
type Input struct {
	Trigger Trigger
	...
	// than derived, so a consumer can prefer stated evidence over inferred.
...
// Has reports whether the outcome contains an effect of the given kind.
func (o Outcome) Has(kind EffectKind) bool {
	return slices.ContainsFunc(o.Effects, func(effect Effect) bool { return effect.Kind == kind })
}

// Dialog is one SIP dialog and everything this edge knows about it. It is NOT safe for concurrent
// use: one goroutine owns one dialog (see the package comment), and the Store hands each pointer to
// exactly one owner.
type Dialog struct {
	...
	// INVITE arriving and the engine answering, which is why no `sip.evt.v1` event may be published
	// before admission (design §4.2).
	OrgID string
	...
	// endpoint. Its presence makes the routing context trunk-capable: the toll-fraud boundary
	// (design §8.3).
	TrunkID string
	...
	// initiator is WHO ended it, which cannot be derived from the reason: a `rejected` is ours when
	// we refused admission and theirs when they answered 486.
	initiator Initiator
	// causeFromReasonHeader records that the Q.850 cause came off an RFC 3326 `Reason` header rather
	// than from a status code. It travels on the terminal event, because a stated cause is better
	...
	// a BYE crossing our BYE is ordinary, and two `leg-ended` events is two CDR rows for one call.
	terminalPublished bool

	// hangupRequested records that the engine has asked for this leg to end. It survives a state
	// change, so a hangup issued while the INVITE was ringing still ends a call a 2xx then answers.
...
// Options constructs a Dialog. Nothing is read from a package-level variable.
type Options struct {
	LegID    string
	OrgID    string
	CallID   string
...
// ErrNoLegID refuses a dialog with no key: a wiring error, since a dialog that cannot be addressed
// is one the engine can never hang up.
...
// the ACK for a UAS leg and the 2xx for a UAC leg, so `billsec` starts where the caller started
// paying.
...
// HangupRequested reports whether the engine has asked for this leg to end, so a second
// `rpc.sip.v1.hangup` is idempotent rather than a second BYE.
func (d *Dialog) HangupRequested() bool { return d.hangupRequested }

// Apply feeds one trigger to the machine and returns what the owner must do. It mutates the dialog
// and touches nothing else, and every refusal is one of the four package errors.
...
// effectsFor is the second half of the machine: what to DO, given that the move is legal. It is
// separate from `transition` so the table stays a table, and it reads the input data — status, SDP,
// cause — that the table deliberately does not see.
func (d *Dialog) effectsFor(in Input, from, next State, at time.Time) []Effect {
	...
	case TriggerLocalEarlyMedia:
		...
		// that same answer byte for byte (RFC 3261 §13.2.1).
	...
	case TriggerLocalAnswer:
		...
		// No `answered` event yet: for a UAS the call is up at the ACK, not at the 200 (design §3.3),
		// and publishing here would start `billsec` before the caller confirmed it heard us.
	...
	case TriggerRemoteCancel:
		...
		// transaction to 487; what it does not do is tell anybody, which is why this trigger exists.
...
// hangupEffects turns `rpc.sip.v1.hangup` into the right method for the state (design §10.3): a BYE
// if confirmed, a CANCEL if we are a UAC in an early dialog, a failure response if we are a UAS
// that has not answered. The engine only says "end this leg with this cause".
func (d *Dialog) hangupEffects(in Input, from State, at time.Time) []Effect {
	...
	d.cause = cause
	// Recorded HERE and not only at the terminal event: the paths that finish a BYE-shaped hangup
	// arrive later with no memory of who asked, and would file it as `timer`.
	if d.initiator == "" {
	...
	case StateInit, StateProceeding, StateEarly:
		if d.Role == RoleUAS {
			// Nothing has been answered, so the end is a failure response on the INVITE transaction.
			// 487 is reserved for a CANCEL we actually received.
			status, reason := StatusForCause(cause)
			d.recordTermination(cause, ReasonRejected, InitiatorLocal, at)
			return append([]Effect{{Kind: EffectRespond, Status: status, Reason: reason}},
				d.terminalEffects(cause, ReasonRejected)...)
	...
	case StateEstablished:
		if d.Role == RoleUAS {
			// RFC 3261 §15: a UAS MUST NOT send a BYE until it has received the ACK for its 2xx, or
			// the far end answers 481 while still trying to ACK and neither side can end the call.
			d.byeDeferred = true
	...
	default: // StateTerminating
		if alreadyRequested {
			// Idempotent on legId (design §4.6): a reissued `hangup` must not produce a second BYE.
			return nil
		}
		return nil
	}
...
func (d *Dialog) remoteAnswerEffects(in Input, from State, at time.Time) []Effect {
	if from == StateEstablished || from == StateConfirmed {
		if in.RemoteTag != "" && in.RemoteTag != d.Identity.RemoteTag {
			// A second 2xx from a different branch: a forking proxy answered twice. RFC 3261
			// §13.2.2.4 still requires the ACK, and the BYE follows because the call is already up
			// on the branch that won; silence would leak a dialog at the far end (design §9.7).
			return []Effect{{
	...
	effects := []Effect{
		...
		// For a UAC the call is up at the 2xx, not at the ACK (design §3.3). The negotiated answer
		// rides the event as its Body so the engine can feed it to `rpc.media.v1.accept-answer`;
		// without it mediad never learns which codec won and the B-leg cannot be bridged. It is the
		// same bytes the ACK carries, and empty on a UAS leg.
	...
	if d.hangupRequested {
		// The hangup raced the answer and the answer won, so the call is torn down properly: ACK
		// first, then BYE, the only sequence the far end accepts (RFC 3261 §13.2.2.4 and §15).
		d.cancelDeferred = false
		effects = append(effects, Effect{
			Kind:   EffectSendBye,
			Cause:  d.cause,
...
func (d *Dialog) timeoutEffects(in Input, from State, at time.Time) []Effect {
	...
	case TimeoutSession:
		// RFC 4028 §10: the refresh never came. The Reason header names the timer so the far end's
		// CDR agrees with ours.
		d.recordTermination(CauseRecoveryOnTimerExpire, ReasonTimeout, InitiatorTimer, at)
		effects := []Effect{
			{Kind: EffectSendBye, Cause: CauseRecoveryOnTimerExpire,
...
// anything. The FIRST recording wins for every field: with a BYE crossing our own BYE, re-stamping
// would make a call the engine ended read as a call the caller ended.
...
// terminalEffects emits the terminal event and releases the claim, exactly once per dialog. The
// guard matters: a BYE crossing our own BYE is ordinary, and two terminal events for one leg is two
...
// isRemoteRequest reports whether a trigger arrived as a SIP REQUEST from the far end, and so has a
// server transaction owed an answer. A CANCEL is deliberately excluded: it has its own refusal path
// (ErrCancelTooLate) with its own status.
