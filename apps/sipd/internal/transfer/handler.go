...
type Options struct {
	...
	// spray that used out-of-dialog REFER would get a budget of its own. Nil disables it.
...
// authorize runs the shared digest pipeline for a REFER and answers the transaction on every
...
func (h *Handler) authorize(
	...
	case registrar.DigestWrongIdentity:
		log.Warn("rejecting a REFER sent as somebody else", "authenticatedAs", result.Auth.Username)
		h.respond(tx, req, statusForbidden, "Forbidden")
	case registrar.DigestThrottled:
		log.Info("rejecting a REFER from a locked source",
