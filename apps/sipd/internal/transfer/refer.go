// Package transfer implements SIP REFER at the edge: the desk phone's TRANSFER key. Signalling
// terminates here, so the edge parses the REFER, decides who may ask, hands it to the engine over
// `rpc.sip.v1.transfer`, and reports the answer back per RFC 3515.
//
// There is deliberately no dialog layer: sipd is a registrar with no INVITE path and no call state,
// so it cannot verify that a REFER's Call-ID names a dialog it is part of. See Handler for what it
// checks instead.
package transfer
...
var (
	...
	// than ignored: ignoring it would silently downgrade an attended transfer to a blind one.
	ErrMalformedReplaces = errors.New("transfer: the Replaces parameter is malformed")
)

// Target is the Refer-To URI, taken apart. User is what the engine dials, because a dial plan
// consumes a destination string and not a URI; Host and URI come along for the log.
type Target struct {
	User string
	Host string
...
// Refer is everything this edge can learn from one REFER. The dialog identifiers are recorded
// VERBATIM and assert nothing: they are a lookup key for the engine, never an authorisation.
type Refer struct {
...
// ParseRefer extracts the transfer request from a REFER. It does no authorisation and reads no
// state: a pure function of the message.
func ParseRefer(req *sip.Request) (Refer, error) {
	...
	// An EMPTY Refer-To reaches here as an empty URI, not a missing header: sipgo normalises
	// `Refer-To:` into `<sip:>`. It is malformed (400), not merely undialable (501).
...
// replacesParam pulls the `Replaces` URI header out of a Refer-To, case-insensitively: sipgo keeps
// URI header names as written and handsets disagree about capitalisation, so an exact-string lookup
// would silently downgrade an attended transfer to a blind one.
