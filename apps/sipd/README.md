...
| **Multi-realm / multi-domain**                                              | **Built.** `Authenticator.ForRequest` challenges for the domain the request NAMES (the To host on a REGISTER, the From host otherwise) and looks the credential up under that realm, so one process serves many tenants. `SIPD_REALM` is the "no tenant matched" default for a request that names no domain, and is logged when it is used. |
...
| `SIPD_PPROF`                           | `false`                 | Serve `net/http/pprof` on the health listener. Requires `SIPD_HEALTH_ADDR` to name a **loopback** host — these handlers dump heap and goroutine state, so a wildcard or external health address is refused at boot.                                                                                                |
