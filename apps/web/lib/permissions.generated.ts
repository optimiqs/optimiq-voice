...
export const PERMISSION_CATALOG: readonly PermissionGroup[] = [
	...
	},
	{
		resource: "messaging",
		label: "Messaging",
		description: "Two-way SMS and MMS on business numbers: the inbox, the act of sending, and the 10DLC brand, campaigns, toll-free verification and opt-out list that make sending legal.",
		permissions: [
			{
				permission: "messaging.read",
				label: "View messages",
				description: "Read conversation threads on the organization's messaging numbers, and see which numbers are registered to send.",
			},
			{
				permission: "messaging.send",
				label: "Send messages",
				description: "Send an SMS or MMS from a registered business number. Blocked automatically for a recipient who has opted out.",
			},
			{
				permission: "messaging.manage",
				label: "Manage messaging registration",
				description: "Enable messaging on a number, register the 10DLC brand and campaigns, submit toll-free verification, and edit the opt-out list.",
			},
		],
	},
	{
	...
	},
	{
		...
		description: "The know-your-customer file, the caller ids this organization may present, and the platform-operator surfaces that review them and answer traceback requests.",
		permissions: [
			{
				permission: "compliance.read",
				label: "View the compliance record",
				description: "Read the organization's KYC file, its onboarding decision, and the caller ids it has a recorded right to present.",
			},
			{
				permission: "compliance.write",
				label: "Manage the compliance record",
				description: "Submit or amend the KYC file, and record an external caller id's verification.",
			},
			{
				permission: "compliance.review",
				label: "Review onboarding decisions",
				description: "Approve, reject or send back any organization's KYC file. Platform operators only.",
			},
			{
				permission: "compliance.traceback",
				label: "Answer traceback requests",
				description: "Query calls to or from a number across every organization, and export the answer. Platform operators only.",
			},
		],
	},
	{
		...
		permissions: [
			...
			},
			{
				...
				description: "Issue a new SIP secret for an extension or a device line, or set one by hand. The old secret keeps working for a short grace period so the handset can pick the new one up; after that the phone stops registering until it does.",
			},
		],
	},
	{
		...
		description: "Spend and velocity limits on international calling, the countries this organization may reach, and the per-extension overrides and suspensions enforced against them.",
		permissions: [
			{
				permission: "toll-fraud.read",
				label: "View fraud controls",
				description: "See the organization's international spend limits, geo rules, current usage against them, and which extensions are suspended.",
			},
			{
				permission: "toll-fraud.write",
				label: "Manage fraud controls",
				description: "Set international minute and concurrency ceilings, allow and deny countries, the off-hours lock, and per-extension overrides. Suspend or restore an extension's outbound calling.",
			},
		],
	},
	{
