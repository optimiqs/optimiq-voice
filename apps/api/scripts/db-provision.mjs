...
if (!databaseUrl) {
  throw new Error("API_DATABASE_URL is required to provision the database");
}
...
async function getAppliedLegacyMigrations(client) {
  ...
  `);
  const { rows } = await client.query(
    'SELECT "name" FROM "optimiq_voice_legacy_migrations"'
  );
...
async function applyLegacyMigrations(client) {
  ...
  if (
    ...
    (await tableExists(client, "applications"))
  ) {
    throw new Error(
      "The API schema exists without migration history; baseline it before provisioning"
    );
  }
  ...
  for (const name of migrations) {
    ...
    const sql = await readFile(
      join(legacyMigrationsFolder, name, "migration.sql"),
      "utf8"
    );
    ...
    try {
      ...
      await client.query(
        'INSERT INTO "optimiq_voice_legacy_migrations" ("name") VALUES ($1)',
        [name]
      );
...
try {
  ...
  if (locked) {
    await client.query(
      ...
      ["optimiq-voice-api-migrations", databaseSchema]
    );
  }
  client.release();
  await pool.end();
