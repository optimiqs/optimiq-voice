...
type VoiceClient = {
	...
	startDtmfGather: (mediaSessionRef: string, callback: (event: { digit: string }) => void) => void;
...
type TextToSpeech = {
	synthesize: (text: string, options: Record<string, unknown>) => { ref: string; stream: Readable };
};
...
export { type GRPCClient, type SpeechToText, type TextToSpeech, type VoiceClient };
