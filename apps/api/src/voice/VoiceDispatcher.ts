...
class VoiceDispatcher {
	...
	async handleStasisStart(event: StasisStart, channel: Channel) {
		...
		try {
			...
			vc.on(SC.PLAYBACK_CONTROL_REQUEST, createPlaybackControlHandler(ari, vc).bind(this));
	...
	async isHandledElsewhere(channel: Channel) {
		return (
			(await createGetChannelVarWithoutThrow(channel)(ChannelVar.FROM_EXTERNAL_MEDIA))?.value ===
			"true"
		);
	}
}
