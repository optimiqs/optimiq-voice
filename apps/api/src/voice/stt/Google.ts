...
class Google extends AbstractSpeechToText<typeof ENGINE_NAME> implements SpeechToText {
	client: SpeechClient;
	...
	async transcribe(stream: Stream): Promise<SpeechResult> {
		...
		const languageCode = this.engineConfig.config.languageCode || VoiceLanguage.EN_US;
