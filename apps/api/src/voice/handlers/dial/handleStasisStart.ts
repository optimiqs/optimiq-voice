...
function handleStasisStart(params: {
	...
	return async (_: undefined, channel: Channel) => {
		try {
			...
			if ([DialRecordDirection.IN, DialRecordDirection.BOTH].includes(recordDirection)) {
				recordChannel(ari, DialRecordDirection.IN, channel.id);
			}

			if ([DialRecordDirection.OUT, DialRecordDirection.BOTH].includes(recordDirection)) {
