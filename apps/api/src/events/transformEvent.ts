...
function transformEvent(event: Record<string, unknown>): Record<string, unknown> {
	...
	if (event.endTime) {
		transformedEvent.endedAt = new Date(event.endTime as string).getTime() / 1000;
	...
	if (extraHeaders && Object.keys(extraHeaders).length > 0) {
		...
		if (extraHeaders[CALL_DIRECTION_HEADER]) {
			transformedEvent.direction = mapCallDirectionToEnum(extraHeaders[CALL_DIRECTION_HEADER]);
		}
	}
