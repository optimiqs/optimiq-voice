...
// The wire half of the direction, tap, mute/hold and negotiation commands. Every case is a
// `[]byte -> []byte` call, because a refusal IS the reply.

func TestAllocateServesEveryDirection(t *testing.T) {
	// Both halves must move: the ANSWER has to say the right thing AND the media plane's own gates have
	// to match it, since answering `sendrecv` to a `sendonly` offer puts a held caller back into a
	// conversation they were taken out of.
	cases := []struct {
		name       string
		offered    string
...
func TestARenegotiationRepointsALiveSession(t *testing.T) {
	// A retry carries the same direction and applying it changes nothing; a re-INVITE carries a
	// different one. `Allocate` stays idempotent either way: no second port, no mode change.
	rig := newRig(t)
...
func TestStartPlaybackServesGeneratedTones(t *testing.T) {
	// A tone needs NO prompt library, so this rig has none: an instance that can bridge a call must
	// still be able to tell the caller the far end is ringing.
...
func TestStartPlaybackServesMusicOnHoldAsALoop(t *testing.T) {
	// `startMusicOnHold` arrives over `start-playback` with a `moh:` reference and stops over
	// `stop-playback` with the same one. The LOOP is derived from the scheme rather than from a flag on
	// the wire, which stops a caller asking for a looping voicemail greeting.
	rig := newRig(t)
	...
	// `stop-playback` carries a reference and nothing else, and a hold loop is a playback.
...
func TestABeepThatCannotPlayDoesNotFailTheRecording(t *testing.T) {
	// A recording that is already running and merely started quietly is a smaller problem than one
	// that did not start at all.
	rig := newRig(t)
	allocateSession(t, rig)
...
func TestTapSessionServesTheThreeSupervisionModes(t *testing.T) {
	// Three features, three argument combinations, one handler — and the SIDES are carried through
	// untouched, because the mixer resolves them and a control surface that reinterpreted them could
	// disagree.
	cases := []struct {
		name    string
...
func TestAllocateNegotiatesWidebandAndOpus(t *testing.T) {
	// G.722 and Opus are both answerable; the difference is what happens LATER — G.722 can be mixed and
	// transcoded, Opus can only be relayed — and it is deliberately invisible here, because an offer is
	// answered on what the two ends can carry.
	cases := []struct {
		name       string
		formats    string
		rtpmap     []string
...
// The state pair — mute and hold — reaching the wire.
...
func TestMuteIsAdditiveAndTheReplyProvesIt(t *testing.T) {
	// Muting `in` on a leg already muted `out` must leave both set, which is why the handler READS the
	// state back instead of deriving it from the request: a direction field that replaced whatever was
	// there is how a conference participant gets their audio back after somebody muted them.
	rig := newRig(t)
	allocateSession(t, rig)
...
func TestHoldResolvesItsMusicBeforeItAnswers(t *testing.T) {
	// A hold that names music READS AND DECODES A FILE first, exactly as `start-playback` does, which is
	// why this subject's deadline is a second where the family's is 500 ms.
	rig := newRig(t)
	allocateSession(t, rig)
...
func TestATapWithNoTargetSideKeepsTheOldConvention(t *testing.T) {
	// ABSENT is a supported state, not a defect: it is what a caller compiled before the field existed
	// sends, and it must behave target-first. The handler leaves it EMPTY rather than defaulting, so
	// `resolveAudiences` is the single place that decision is written down.
	rig := newRig(t)
	allocateSession(t, rig)
