...
/**
 * Column definitions for rendering a table of Optimiq Voice Domains using TanStack Table.
 *
...
export const columns: ColumnDef<Domain>[] = [
	...
	},
	{
		...
		cell: ({ row }) => {
			...
			const policies = (row.getValue("egressPolicies") as Domain["egressPolicies"]) || [];

			// Format policies into a readable string: "rule (numberRef)"
			const label = policies.map(({ rule, numberRef }) => `${rule} (${numberRef})`).join(", ");
