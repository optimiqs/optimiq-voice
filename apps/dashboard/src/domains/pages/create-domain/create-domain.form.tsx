...
export function CreateDomainForm({
	...
	const selectValues = useMemo(() => {
		return policies.map((item) => `${getNumberName(item.numberRef)} (${item.rule})`);
	...
	const handleCloseRulesModal = useCallback(() => setIsRulesModalOpen(false), []);
	const handleCloseAclsModal = useCallback(() => setIsDomainAclsModalOpen(false), []);
