import { ApplicationType, type CreateApplicationRequest } from "@optimiq-voice/types";
import { Logger } from "~/core/shared/logger";
import type { Form, Schema } from "../pages/create-application/schemas/application-schema";

export function formatApplicationData({ intelligence, ...data }: Schema, form: Form) {
	...
	if (data.type === ApplicationType.AUTOPILOT && intelligence) {
		...
		if (!application.intelligence.productRef) {
			form.setError("intelligence.config.languageModel.provider", {
				type: "required",
				message: "Language model provider is required for AUTOPILOT applications",
			});
			return;
		}
