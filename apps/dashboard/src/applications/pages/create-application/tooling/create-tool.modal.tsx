...
const schema = z.object({
	...
	operation: z.object({
		...
		headers: z
			.array(z.object({ key: z.string().nonempty(), value: z.string().nonempty() }))
			.default([]),
	}),
});
...
const Section = ({ title, description }: SectionProps) => (
	<Box sx={{ mt: "8px" }}>
		<div style={{ display: "flex", flexDirection: "column", gap: 2 }}>
			...
			{description && (
				<span style={{ fontSize: 11, color: "var(--mui-palette-base-03)" }}>{description}</span>
			)}
		</div>
	</Box>
);
...
export const CreateToolModal = ({ isOpen, onClose, onFormSubmit, initialValues }: ModalProps) => {
	...
	const transformHeadersToObject = useCallback((headersArray: Schema["operation"]["headers"]) => {
		return Object.fromEntries(headersArray.map((h) => [h.key.toLowerCase(), h.value]));
	}, []);

	const extractRequiredKeys = useCallback(
		(propertiesArray: Schema["parameters"]["properties"], requiredArray: string[]) => {
		...
		(prop: any, idx: number) => (
			...
			>
				...
				<Box sx={{ display: "flex", gap: 1 }}>
					<Button size="small" variant="outlined" onClick={() => handleEditParameter(idx)}>
						Edit
					</Button>
					<Button size="small" variant="outlined" onClick={() => handleRemoveParameter(idx)}>
						Remove
					</Button>
				</Box>
			</Box>
