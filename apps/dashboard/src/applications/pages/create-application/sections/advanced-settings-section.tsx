...
import { getLanguageModelModels, LANGUAGE_MODEL_PROVIDERS } from "../create-application.const";
			...
			<AccordionDetails sx={{ display: "flex", flexDirection: "column", gap: "24px" }}>
				...
				<Box sx={{ mt: "8px" }}>
					...
					<Typography variant="body-micro" color="base.03">
						For call transfer, please provide a phone number. Also, optionally, you can set a
						message to be used when the call is transferred and a timeout for the transfer operation
						(in milliseconds).
					</Typography>
				</Box>
				<FormField
					...
					render={({ field }) => (
						<FormItem>
							<FormControl>
								<Input type="text" label="Phone Number" placeholder="e.g. +1234567890" {...field} />
							</FormControl>
						</FormItem>
					)}
				/>
				...
				<Box sx={{ mt: "8px" }}>
					...
					<Typography variant="body-micro" color="base.03">
						This is the message that will be played when there is no activity in the call. You can
						also set a timeout (in milliseconds) and maximum timeout count for idle handling.
					</Typography>
				</Box>
				<FormField
					control={control}
				...
				<Box sx={{ display: "flex", flexDirection: "column", gap: "4px" }}>
					...
					<Typography variant="body-micro" color="base.03">
						When enabled, the user can interrupt the agent by speaking. When disabled, background
						noise is less likely to interrupt the agent, but the user cannot interrupt while the
						agent is speaking.
					</Typography>
					<FormField
						control={control}
				...
				<Box>
					...
					<Typography variant="body-micro" color="base.03">
						Select the language model provider and model that best fit your use case. If there is an
						additional model you would like us to support, please let us know.
					</Typography>
				</Box>
				<FormField
					control={control}
					...
					render={({ field }) => (
						<FormItem>
							<FormControl>
								<Select label="Provider" options={LANGUAGE_MODEL_PROVIDERS} {...field} />
							</FormControl>
						</FormItem>
					)}
				/>
				...
				<Box sx={{ mt: "8px" }}>
					...
					<Typography variant="body-micro" color="base.03">
						Use this to receive events from Optimiq Voice. You can select the events you want to
						receive. You will need to provide a URL to receive the events.
					</Typography>
				</Box>
				<FormField
					control={control}
