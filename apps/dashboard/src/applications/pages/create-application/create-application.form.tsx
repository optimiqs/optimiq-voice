...
import { type Form as FormType, type Schema, resolver } from "./schemas/application-schema";
...
export const CreateApplicationForm = ({
	...
	const languageModelProvider = form.watch("intelligence.config.languageModel.provider");
	...
	const prevLanguageModelProviderRef = useRef<string | undefined>(languageModelProvider);
	...
	useEffect(() => {
		...
		if (languageModelProvider && languageModelProvider !== prevLanguageModelProviderRef.current) {
	...
	return (
		<Form {...form}>
			<FormRoot onSubmit={form.handleSubmit(onFormSubmit)}>
				...
				/>
				{isAutopilot && (
					<ConversationSettingsSection control={form.control} isAutopilot={isAutopilot} />
				)}
				<SpeechSection control={form.control} isAutopilot={isAutopilot} ttsVendor={ttsVendor} />
				{isAutopilot && (
