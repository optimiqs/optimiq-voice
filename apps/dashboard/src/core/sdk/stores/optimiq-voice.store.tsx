 ...
 */
export const OptimiqVoiceContext = createContext<OptimiqVoiceContextValue | null>(null);
 ...
 */
export const OptimiqVoiceProvider = ({ children, initialSession }: OptimiqVoiceProviderProps) => {
	/**
	 ...
	 */
	const { sdk, client, session, setSession, isAuthenticated, logout, updateSessionTokens } =
		useClient();
	...
	const authenticate = useCallback(
		async (sessionToAuth: CookieSession) => {
			...
			Logger.debug("[<OptimiqVoiceProvider />] Session authenticated successfully.");
	...
	useEffect(() => {
		...
		Logger.debug("[<OptimiqVoiceProvider />] Initializing Optimiq Voice client...");
	 ...
	 */
	return <OptimiqVoiceContext.Provider value={value}>{children}</OptimiqVoiceContext.Provider>;
};
