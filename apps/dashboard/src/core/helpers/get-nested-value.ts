 ...
 */
export function getNestedValue(obj: Record<string, any>, key: string): string | undefined {
	const data = key.split(".").reduce((acc, part) => acc?.[part], obj);

	return data !== undefined ? (Array.isArray(data) ? data.join(", ") : String(data)) : undefined;
}
