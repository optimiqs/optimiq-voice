import { Box, Paper, ClickAwayListener, styled, Fade, Grow } from "@mui/material";
...
export const FilterSearchBySelector: React.FC<FilterSearchBySelectorProps> = ({
	...
	const selectedFilterLabel =
		searchableFields.find((option) => option.value === searchBy)?.label || placeholder;
		...
		const dropdown = (
			<UnifiedDropdown>
				...
				{searchableFields.map((option) => (
					<FilterOption key={option.value} onClick={() => handleFilterSelect(option.value)}>
						{option.label}
					</FilterOption>
				))}
			</UnifiedDropdown>
