...
import { TextareaInput, TextareaRoot, TextareaFormControl } from "./textarea.styles";
	...
	(
		...
		return (
			<TextareaFormControl fullWidth error={Boolean(error)}>
				{label && (
					<InputLabel shrink className="MuiFormLabel-root MuiInputLabel-root MuiInputLabel-shrink">
						{label}
					</InputLabel>
				)}

				<TextareaRoot size={size}>
					{leadingIcon && <InputAdornment position="start">{leadingIcon}</InputAdornment>}
					<TextareaInput
						ref={ref}
						minRows={minRows}
						maxRows={maxRows}
