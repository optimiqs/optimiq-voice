...
export function useOptimisticDeleteResource<TRef extends string = string>(
	...
	>({
		...
		onMutate: async (refToDelete: TRef) => {
			...
			Logger.debug("[useOptimisticDeleteResource] Deleting optimistically for: ", refToDelete);
			...
			const previousItem = queryClient.getQueryData<BaseApiObject>([...resourceKey, refToDelete]);
		...
		onError: (_err, ref, context) => {
			Logger.error("[useOptimisticDeleteResource] Error deleting resource: ", ref, _err);

			if (context?.previousList) {
				Logger.debug("[useOptimisticDeleteResource] Restoring previous list state for: ", ref);
				queryClient.setQueryData(collectionKey, context.previousList);
			}

			if (context?.previousItem) {
				Logger.debug("[useOptimisticDeleteResource] Restoring previous item state for: ", ref);
