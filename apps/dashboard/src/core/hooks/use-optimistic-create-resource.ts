...
>(
	...
	return useMutation<TData, unknown, TVariables, { tempId: string }>({
		 ...
		 */
		onError: () => {
			Logger.debug("[useOptimisticCreateResource]: Mutation failed, invalidating collection cache");
		...
		onSuccess: (resource, _vars, context) => {
			...
			Logger.debug("[useOptimisticCreateResource]: Updating cache with new resource", resource);
