...
export function meta(_: Route.MetaArgs) {
	return [{ title: "Workspace Settings | Optimiq Voice" }];
}
...
export default function Overview() {
	...
	const { mutate: removeMember, isPending: isLeaving } = useWorkspaceRemoveMember();
	...
	const handleConfirmLeave = useCallback(() => {
		if (!user || !currentWorkspace) {
			toast("Oops! We are unable to complete this action. Please try again later.");
			return;
		}
			...
			onSuccess: () => {
				queryClient.setQueryData<ListResponse<Workspace> | undefined>(
					...
					(previous) => {
						...
						return {
							...previous,
							items: previous.items.filter((workspace) => workspace.ref !== currentWorkspace.ref),
						};
					},
				);
					...
					{isOwner && (
						<Box sx={{ mt: 6 }}>
							...
							<Stack spacing={2}>
								...
								>
									<Button variant="outlined" size="small" onClick={handleOpenDeleteDialog} danger>
										Delete Workspace
									</Button>
								</Box>
							</Stack>
					...
					{!isOwner && currentWorkspace && (
						<Box sx={{ mt: 6 }}>
							...
							<Stack spacing={2}>
								...
								<Typography variant="body-small" color="base.03">
									Leave this workspace if you no longer need access. You can be invited again by a
									workspace owner.
								</Typography>
								...
								>
									<Button variant="outlined" size="small" onClick={handleOpenLeaveDialog} danger>
										Leave Workspace
									</Button>
								</Box>
							</Stack>
