 ...
 * It retrieves the ID token from Optimiq Voice, decodes it to extract the user ID,
...
export const useCurrentUser = () => {
	...
	/**
	 * Get the Optimiq Voice client to retrieve the ID token.
	 */
	const { client, session } = useOptimiqVoice();
	...
	useEffect(() => {
		...
		if (!token) {
			Logger.debug("[useCurrentUser()]: Oops! No ID token found. User ID cannot be set.");
			return;
		}
		...
		try {
			...
			Logger.error("[useCurrentUser()]: Oops! Failed to decode ID token.", error);
		}
	}, [client, session]);
