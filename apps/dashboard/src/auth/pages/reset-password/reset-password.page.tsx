...
import { ResetPasswordForm, type Form, type Schema } from "./reset-password.form";
...
export default function ResetPasswordPage() {
	...
	const onSubmit = useCallback(async ({ password }: Schema, form: Form) => {
		...
		try {
			...
			if (Object.keys(payload).length !== 2 || !payload.username || !payload.code) {
		...
		<Box width="100%" maxWidth="440px" gap="40px" display="flex" flexDirection="column">
			<Box gap="16px" display="flex" flexDirection="column" textAlign="center">
				...
				<Typography variant="body-small" color="base.03">
					Please reset your password using 8+ characters with upper, lower, number, and symbol.
				</Typography>
			</Box>
