...
export default class List extends AuthenticatedCommand<typeof List> {
	static override readonly description = "display all API keys in the active Workspace";
	...
	public async run(): Promise<void> {
		...
		response.items.forEach((application) => {
			ui.div(
				...
				{
					text: application.expiresAt ? new Date(application.expiresAt).toISOString() : "Never",
					padding: [0, 0, 0, 0],
					width: 10,
				},
			);
