...
export default class Create extends AuthenticatedCommand<typeof Create> {
	...
	public async run(): Promise<void> {
		...
		const domainsList = (await domains.listDomains({ pageSize: 1000 })).items.map((d) => ({
			name: d.name,
			value: d.ref,
		}));

		const credentialsList = (await credentials.listCredentials({ pageSize: 1000 })).items.map(
			(c) => ({ name: c.name, value: c.ref }),
		);
