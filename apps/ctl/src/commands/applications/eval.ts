...
import type { EvaluateIntelligenceEvent } from "@optimiq-voice/types";

export default class EvalIntelligence extends AuthenticatedCommand<typeof EvalIntelligence> {
	static override readonly description = "experimental command to test an Autopilot application";
	...
	public async run(): Promise<void> {
			...
			(scenario) => {
				scenario.conversation.map((step) => {
					if (step.expected?.text?.type) {
						...
						step.expected.text.type = type === "similar" ? ExpectedTextType.SIMILAR : "EXACT";
					}
				});
				return scenario;
			},
		...
		rawAutopilotApplication.intelligence.config.testCases.scenarios = mappedScenarios;
