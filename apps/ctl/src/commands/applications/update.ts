...
import { UpdateApplicationRequest } from "@optimiq-voice/types";
...
export default class Update extends AuthenticatedCommand<typeof Update> {
	static override readonly description = "modify the configuration of an Application";
	...
	public async run(): Promise<void> {
		...
		if (flags["from-file"]) {
			try {
				...
				await createOrUpdateApplication(client, flags["from-file"] as string, ref, true);
				this.log("Done!");
				return;
			} catch (e) {
